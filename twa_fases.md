...
- **Stappen:**
  ...
  - **TWA-bevinding (juni 2026):** swipe-navigatie werkte niet op Android Chrome/TWA (op iOS wél) — Android Chrome vuurt `pointercancel` zodra het een touch-drag voor native scroll claimt, waardoor de `pointerup` van `useSwipe` nooit komt. `touch-pan-y` op `<main>` alleen bleek niet genoeg; definitieve fix: `useSwipe` omgebouwd van pointer-events naar **touch-events** (`touchstart`/`touchend` lopen tijdens native scroll door en krijgen geen cancel), met dezelfde drempels/guards. **Op het toestel bevestigd werkend.** Tevens zijn de **push-notificatieteksten emoji-vrij** gemaakt (titel "Herinnering"/"Verjaardag"/"Testmelding"); e-mail/Telegram ongemoeid.
- ✅ **Gebouwd én op het toestel bevestigd** (juni 2026): TWA opent fullscreen zonder URL-balk, login/push/swipe werken; AAB ligt klaar voor een evt. latere Play-upload. Resterende toestel-tests (Lighthouse, offline, volledige pushchecklist) via `toestel_tests.md`.
