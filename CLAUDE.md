...
- **Reistijd-buffer (juni 2026)** — vierde idee uit `internet_ideas.md`. Per getimed event een handmatige **`reistijdMinuten`** (DB-kolom, migratie in de Database-sectie; **géén route-API, geen locatie/tracking**). Instellen via de rij "Reistijd" in `AfspraakFormulier` (alleen bij niet-heeldag): select Geen/5/10/15/30 min/1 uur + "Aangepast…" (vrij veld 1–480 min, ongeldig → genegeerd/rode hint) met live "Vertrek om HH:MM" + privacy-helptekst. **Schaduwblok** in `DagWeergave`/`WeekWeergave`: een gestreept (`dashed`, transparant, lage opacity, accentkleur) blok direct vóór het event (top geclamp op 0 bij vroege ochtend), klikbaar → opent het event (`z-0`, dus onder het echte event-blok; werkuren-overlay eronder en tijd-indicator z-20 erboven ongemoeid); herberekent vanzelf na drag/undo (volgt uit `beginTijd`), nooit als los event opgeslagen. Nieuw menu-item **"Reistijd"** (lucide `Car`) in Sidebar + MobielMenu met een **dashed iOS-separator** (item met `sep`-flag krijgt `border-t border-dashed` erboven) → `ReistijdModal.tsx`: aankomende events met reistijd, gesorteerd op vertrekmoment, "Vertrek om 08:40 · afspraak om 09:00", klik = navigeren + openen. Fail-open vóór de migratie via `upsertAfspraken()` (laat `reistijd_minuten` weg bij de retry). **Dupliceren behoudt** reistijd (anders dan favoriet); "hele reeks bewerken" raakt het niet (per occurrence). **Vertrektijd-reminder bewust uitgesteld** (vereist cron-query-tak + tweede firing) — vervolgstap in ideas.md.
...
- Kolom `favoriet` toevoegen voor de Countdowns-feature (client valt fail-open terug zonder deze kolom):
  ...
  ```
- Kolom `reistijd_minuten` toevoegen voor de reistijd-buffer (client valt fail-open terug zonder deze kolom):
  ```sql
  alter table afspraken add column if not exists reistijd_minuten integer default 0;
  ```
