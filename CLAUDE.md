...
- **TWA / Google Play Store (gepland)** — onderzoek + fasering in `twa_fases.md`. Doel: de PWA via Bubblewrap als Trusted Web Activity in een **gesloten Play-testtrack** (package `nl.jellebol.agenda`, appnaam "Agenda", bestaande Web Push hergebruikt — geen FCM). Belangrijkste voorwerk: manifest `scope`/`id`, echte maskable icon (huidige is een 1-op-1 kopie zonder safe zone), nieuw icoon-ontwerp, privacypagina (`/privacy`), `public/.well-known/assetlinks.json`, Play-account. Nog niet gestart.
