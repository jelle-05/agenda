...
import {
  getWeekDagen, toISODatum, isVandaag, getDagIndex, isSameDag,
  NL_DAGEN_KORT, tijdNaarMinuten,
} from '@/lib/datum'
...
export default function WeekWeergave({ huidigeDatum, afspraken, labels, onDagKlik }: Props) {
        ...
        <div className="flex" style={{ height: `${24 * UURHOOGTE}px`, minHeight: `${24 * UURHOOGTE}px` }}>
          ...
          {weekDagen.map((dag, di) => {
            ...
            const dagAfspraken = afspraken
              .filter((a) => a.datum === iso && !a.heeldag)
              ...
              >
                ...
                {dagAfspraken.map((afspraak) => {
                  ...
                  return (
                    <div
                      key={afspraak.id}
                      className="absolute inset-x-0.5 rounded overflow-hidden cursor-pointer px-1 py-0.5"
                      style={{
                        top,
                        height,
