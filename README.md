...
- **Weer bij de dagkoppen** — optioneel (Instellingen → Kalender): klein weericoon + maximumtemperatuur in de dag- en weekweergave voor de komende ~7 dagen, via Open-Meteo (geen API-key). Op basis van een vaste, zelf ingestelde locatie — er wordt geen live locatie gevolgd; bij offline of een API-fout verdwijnt het weer gewoon stilletjes
- **Weer-detailoverzicht** — via "Weer" in de zijbalk (desktop) of het menu (mobiel): vandaag in detail met uurverloop (temperatuur en regenkans per uur) plus de komende ~7 dagen met min/max en regenkans. Het dagelijkse dagoverzicht (mail/Telegram) vermeldt ook het weer en de regenkans met tijdstip
- **Zoeken** — via het zoekicoon (desktop) of het menu (mobiel): doorzoekt titel, locatie en notities van al je afspraken én de namen van je verjaardagen (ook accent-ongevoelig); een resultaat aanklikken navigeert ernaartoe en opent het event of de verjaardag
...
- **Reistijd-buffer** — stel per afspraak een reistijd in (presets of een eigen aantal minuten); de app toont "Vertrek om 08:40", een gestreept schaduwblok vóór de afspraak in de dag-/weekweergave, en een overzicht van vertrektijden via het menu-item "Reistijd". Volledig handmatig — er wordt geen route, locatie of live positie opgehaald
- **Countdowns** — markeer een afspraak of verjaardag als favoriet via de ster in het formulier en zie in het menu-item "Countdowns" hoe lang het nog duurt ("Vakantie over 23 dagen", "Lisa over 12 dagen"); verjaardagen tellen af naar de eerstvolgende editie, verlopen favoriete afspraken staan compact onder "Voorbij" en kun je daar uitzetten
...
- **Instellingenpagina** — modal met tabstructuur (`InstellingenMenu`, tabs "Voorkeuren" / "Notificaties" / "Account"): test-e-mailreminder sturen en Telegram beheren (koppelen, testbericht, **reminders aan/uit**, ontkoppelen)
- **Wachtwoord wijzigen** — in Instellingen → Account: nieuw wachtwoord + bevestiging (minimaal 6 tekens); je blijft ingelogd na het wijzigen
- **Voorkeuren** — tab in Instellingen, gesynchroniseerd via je account: **Open kalender met** (automatisch = week op desktop/dag op mobiel, laatst gebruikte weergave, of een vaste keuze), **standaard herinnering** en **standaard duur** voor nieuwe afspraken. Wijzigingen worden direct opgeslagen; bestaande afspraken veranderen nooit mee
- **Dagoverzicht** — optioneel (standaard uit): elke ochtend op een zelfgekozen tijdstip (standaard 07:00) een kort overzicht van je afspraken en verjaardagen van vandaag, via **Telegram** (koppeling vereist) of **e-mail**. Bewust minimalistisch: platte tekst, geen emoji's of opmaak. Instelbaar in Voorkeuren, inclusief een optionele naam voor de begroeting; maximaal één bericht per dag
...
- **Browser-meldingen aan/uit per apparaat**: sectie "Meldingen" in Instellingen → Notificaties — aanzetten (permissie + abonnement), uitzetten op dit apparaat, en uitleg als meldingen in de browser geblokkeerd zijn
- **Dode push-abonnementen worden automatisch opgeruimd**: geeft een push-dienst `404`/`410` terug (abonnement verlopen of ingetrokken), dan verwijdert de cron dat abonnement; tijdelijke fouten laten het abonnement staan
- **Geblokkeerde Telegram-bot wordt automatisch ontkoppeld**: geeft Telegram een `403` terug (bot geblokkeerd), dan verwijdert de cron de koppeling — reminders vallen automatisch terug op browser-push en je kunt opnieuw koppelen via Instellingen; faalt een Telegram-poging, dan wordt de reminder in dezelfde run alsnog via push bezorgd
